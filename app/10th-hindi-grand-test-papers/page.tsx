"use client"

import Link from "next/link"
import { ArrowLeft, FileSpreadsheet, Eye, Download } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"

export default function TenthHindiGrandTestPaper1() {
  return (
    <div className="min-h-screen bg-white">

      {/* Hero Section */}
      <section className="bg-gradient-to-br from-red-50 to-orange-50 py-16">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-5xl font-bold mb-4">
            Class 10 <span className="text-red-600">Grand Test Paper 1</span>
          </h1>
          <p className="text-xl text-gray-600">
            10th Class Hindi Grand Test (2025-26)
          </p>
        </div>
      </section>

      {/* Grand Test Paper Section */}
      <section className="py-16">
        <div className="container mx-auto px-4 max-w-4xl">

          <h2 className="text-2xl font-bold text-center mb-10">
            Grand Test Papers
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">

            {/* Grand Test Paper 1 */}
            <Card className="hover:shadow-lg border-2 transition duration-300">
              <CardContent className="p-6">

                <div className="flex gap-4 mb-4">
                  <div className="w-12 h-12 rounded-full flex items-center justify-center bg-red-600">
                    <FileSpreadsheet className="text-white" />
                  </div>
                  <div>
                    <h3 className="font-semibold text-lg">
                      Grand Test Paper 1
                    </h3>
                    <p className="text-sm text-gray-500">
                      BALARAM 10th Hindi Grand Test (2025-26)
                    </p>
                  </div>
                </div>

                {/* View Button */}
                <Link
                  href="/pdfs/Balaram-10-Hindi-Grand Test-KEY.pdf"
                  target="_blank"
                >
                  <Button className="w-full mb-2 bg-red-600 hover:bg-red-700 text-white">
                    <Eye className="mr-2 h-4 w-4" />
                    View Paper
                  </Button>
                </Link>

                {/* Download Button */}
                <a
                  href="/pdfs/Balaram-10-Hindi-Grand Test-KEY.pdf"
                  download
                >
                  <Button variant="outline" className="w-full">
                    <Download className="mr-2 h-4 w-4" />
                    Download PDF
                  </Button>
                </a>

              </CardContent>
            </Card>

          </div>

          {/* Back Button */}
          <div className="text-center mt-12">
            <Link href="/10th-hindi">
              <Button className="bg-red-600 hover:bg-red-700 text-white px-8 py-3">
                <ArrowLeft className="mr-2 h-5 w-5" />
                Back to 10th Hindi Resources
              </Button>
            </Link>
          </div>

        </div>
      </section>
    </div>
  )
}